...
## data ####
# read actual data
xtrain <- read_csv(paste("D:/Homsite/xtrain_",dataset_version,".csv", sep = ""))
xtest <- read_csv(paste("D:/Homsite/xtest_",dataset_version,".csv", sep = ""))
...
xfolds <- read_csv("D:/Homsite/xfolds.csv"); xfolds$fold_index <- xfolds$fold5
...
for (ii in 1:nrow(param_grid))
{
  ...
  for (jj in 1:nfolds)
  {
    ...
    x0 <- xtrain[isTrain,]; x1 <- xtrain[isValid,-276]
